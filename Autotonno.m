function varargout = Autotonno(varargin)
% AUTOTONNO M-file for Autotonno.fig
%      AUTOTONNO, by itself, creates a new AUTOTONNO or raises the existing
%      singleton*.
%
%      H = AUTOTONNO returns the handle to a new AUTOTONNO or the handle to
%      the existing singleton*.
%
%      AUTOTONNO('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in AUTOTONNO.M with the given input arguments.
%
%      AUTOTONNO('Property','Value',...) creates a new AUTOTONNO or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Autotonno_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Autotonno_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Autotonno

% Last Modified by GUIDE v2.5 23-Jul-2018 19:03:22

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Autotonno_OpeningFcn, ...
                   'gui_OutputFcn',  @Autotonno_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Autotonno is made visible.
function Autotonno_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Autotonno (see VARARGIN)

% Choose default command line output for Autotonno
handles.output = hObject;

handles.version = '0.7.77';
...
set(handles.axes2,'ButtonDownFcn','Autotonno(''mouseclick'',gcbo,[],guidata(gcbo))');
...
% UIWAIT makes Autotonno wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = Autotonno_OutputFcn(hObject, eventdata, handles) 
...
% --- Definition of objects and variables w/ default values
...
handles.scale_options.indices = [];             % Variables definition
handles.scale_options.freqs = [];
handles.scale_options.notes = {};
handles.scale_options.fund_index = [];

% Pitch detection defaults
handles.pitch_options.Fmin = 50;             	% Min (fundamental) frequency to search
handles.pitch_options.Fmax = 600;            	% Max (fundamental) frequency to search
handles.pitch_options.block_length = 0.04;   	% Time slot (s) to analyze for frequency
handles.pitch_options.step_per_block = 2;       % What fraction of the block width to step for each pitch detect (OLA)
handles.pitch_options.threshold_amp = 0.25;     % Threshold of autocorrelation to be considered a peak
handles.pitch_options.harmonic_deviation = 0.2; % Max deviation (%) from multiple fundamental to be considered a harmonic
handles.pitch_options.threshold_ratio = 0.6;    % Max ratio in heights of autocorrelation allowed between fundamental and harmonics
handles.pitch_options.f0_target_sample_time = 0.08; % Over what time interval to sample previous freq's to determine next target f0 (BOH, MATHWORKS)
handles.pitch_options.target_tol = 0.3;         % Max tolerance (%) from f0 target 
...
% Initialize objects for record, import and playback
handles.record_obj = [];
handles.import_obj = [];
handles.play_obj = [];
...
set(handles.edit_savewav,'String','untitled.wav');


% --- Update of what is plot on-screen
...
if val == 1
    if ~isempty(handles.sound.t) && ~isempty(handles.sound.A)
        ...
        % Plot of original and corrected signals
...
elseif val == 2 % Multiple point selection
...
% --- Update of the graphical interface and buttons status
...
if handles.status.isrecording
    ...
    set(handles.import_button,'enable','off');
    set(handles.savewav_button,'enable','off');
elseif handles.status.isplaying
    ...
    set(handles.savewav_button,'enable','on');
end
    

% Set all apply buttons to disabled until there is a frequency vector (for
% initialization)
...
if handles.status.isreset % (Better keep separated)
    for N = 1:7
...
if handles.status.isreset || handles.status.issave
    if isempty(handles.status.filename)
        set(handles.figure1,'Name','Autotonno™ (Pitch Detection, Correction and Frag)');
    else
        [pathstr, name, ext, versn] = fileparts(handles.status.filename);
        set(handles.figure1,'Name',sprintf('Autotonno™ [%s]',name))
    end
    handles.status.issave = false;
end
...
% Functions designed to respond to interaction with GUI. They constitute
% the core of the AUTOTONNO.
% In GUIDE: change callback from object property. NB: Callback name in 
% object property must match the name in Autotonno.m file!
% (Normally, empty callbacks are generated when an object is created)
...
try
    ...
    h = errordlg('Error starting audio input device: Check sound card and microphone! (Gogliò)','ERROR');
    waitfor(h);
    return
end
...
if handles.status.isrecording
    ...
    % Trim initial samples (thank you Mathworks community)
    ...
    % Clear old frequency data (new file!)
    handles.sound.f0 = [];
    handles.sound.f0_save = [];
    handles.sound.f0_corrected = [];
    handles.sound.f0_corrected_save = [];
...
[file,path] = uigetfile('./*.wav', 'Select the audio file (.wav ONLY)', 'MultiSelect', 'off'); 
[audio_signal,audio_fs] = audioread([path,file]);

% Get the waveform
handles.sound.A = audio_signal;
    
% Save sampling frequency
handles.sound.Fs = audio_fs;

% Calculate time vector
handles.sound.t = (0:length(handles.sound.A)-1)./audio_fs;
...
% --------------------------------------------------------------------
function savewav_button_Callback(hObject, eventdata, handles)
...
set(hwait,'Name','Autotonno says: Please Wait');
waitbar_count = 0;
...
for n = 1:N
    ...
    % Get the waveform
    if ~isempty(handles.sound.A_corrected)
        audio_signal = handles.sound.A_corrected;
    else
        audio_signal = handles.sound.A;
    end
    
    % Get sampling frequency
    audio_fs = handles.sound.Fs;
    ...
    % Save .wav file
    filename = get(handles.edit_savewav,'String');
    audiowrite(filename,audio_signal,audio_fs);
end
...
% Decide which to play, original or modified
...
% Draw vertical line (for scan effect)
axes(handles.axes1);
...
% UNTITLED FUNCTIONS: toolbar, do not know why but if I remove them the
% program explodes.
...
% 
% Iu = find(abs(handles.scale_options.freqs - mean_f0) <= min(abs(handles.scale_options.freqs - mean_f0)));
...
for index = 1:length(f0)
   Iu = find(abs(handles.scale_options.freqs - f0(index)) <= min(abs(handles.scale_options.freqs - f0(index))));
   
   if ~isnan(f0(index))
    f0(index) = handles.scale_options.freqs(Iu);
   end
...
% Find direction of snap (up/down)
...
% To be implemented. I wanted to make the final presentation pop up but it
% just do not work. Do not why, ubuntu?
...
% (This is the core of the program <3)
...
f0_samples = floor(f0_target_sample_time/(step/Fs));
...
for n = 1:N
    ...
    [f0(n),acorr,Nshifts,Tshifts] = find_f0_timedomain2(Atemp,Fs,Fmin,Fmax,...
...
elseif get(handles.plot_selection,'value') ~= 1
    ...
    elseif strcmp(clicktype,'extend')
        ...
        handles.tstart = now;
        guidata(hObject, handles);
        set(gcf,'WindowButtonMotionFcn','Autotonno(''wbmcb'',gcbo,[],guidata(gcbo))');
        set(gcf,'WindowButtonUpFcn','Autotonno(''wbucb'',gcbo,[],guidata(gcbo))');
...
% --- It's the autocorrelazione man!
% This is where the magic happens. Thanks Mathworks community.
...
% INPUTS:
%   
%   A:          The input signal A(t) [block], sampled at an interval Ts
%
...
% figure(2); clf
% plot(acorr(1:index-1),'b--');
% hold on

% Try to make the autocorrelation "level" and with the minimum at zero
level = polyval(polyfit(Nshifts,acorr,1),Nshifts);
acorr = acorr - level;
% plot(level,'r');
...
% --- Support function to find peaks above a certain threshold
...
function edit_savewav_Callback(hObject, eventdata, handles)
% hObject    handle to edit_savewav (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_savewav as text
%        str2double(get(hObject,'String')) returns contents of edit_savewav as a double
if isempty(handles.sound.A_corrected)
    handles = run_pitch_correction(handles);
end
...
% --- Executes during object creation, after setting all properties.
function edit_savewav_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_savewav (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
